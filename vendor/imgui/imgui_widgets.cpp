...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2( 20.0f, 0.0f );
    ...
    RenderFrame(check_bb.Min, check_bb.Max, *v ? ImColor( 70, 81, 107, 255 ) : ImColor( 46, 51, 64, 255 ), true, 2.0f );
    ...
    if ( label_size.x > 0.0f )
        window->DrawList->AddText( label_pos, ImColor( 194, 194, 194, 255 ), label );//RenderText(label_pos, label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2( 20.0f, 0.0f ), window->DC.CursorPos + ImVec2( w, label_size.y + 6.0f ) );
    ...
    window->DrawList->AddRectFilled( bb.Min, bb.Max, ImColor( 50, 56, 71, 255 ), 2.0f );
    window->DrawList->AddRect( bb.Min, bb.Max, ImColor( 50, 56, 71, 255 ), 2.0f );

    if( !popup_open )
        RenderArrow( bb.Max - ImVec2( 16, 12 ), ImGuiDir_Down, 0.5f );
    else
        RenderArrow( bb.Max - ImVec2( 16, 12 ), ImGuiDir_Up, 0.5f );
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2( 20.0f, 0.0f ), window->DC.CursorPos + ImVec2( w, label_size.y + 10.0f ) );
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddText(frame_bb.Min, ImColor( 194, 194, 194, 255 ), label );
    if (temp_input_is_active)
    {
        // Only clamp CTRL+Click input when ImGuiSliderFlags_AlwaysClamp is set
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(slider_bb.Min + ImVec2( 1.0f, 1.0f ), grab_bb.Max + ImVec2( 1.0f, 1.0f ), ImColor( 70, 81, 107, 255 ), 2.0f );
    ...
    window->DrawList->AddText( ImVec2( frame_bb.Min.x + ( slider_bb.Max.x - slider_bb.Min.x - CalcTextSize( value_buf, NULL, true ).x ), frame_bb.Min.y ), ImColor( 194, 194, 194, 255 ), value_buf, value_buf_end );
